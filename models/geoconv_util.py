...
from tf_geoconv import aggregate
from tf_util import perceptron, batch_norm_for_fc, batch_norm_for_conv1d
...
def pointnet_sa_module_msg_V2(xyz, points, npoint, radius_list, nsample_list, mlp_list, 
        mlp2, mlp2_bypass, mlp2_radius, is_training, bn_decay, scope, bn=True, use_xyz=True, use_nchw=False):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for j, num_out_channels in enumerate(mlp2):
            new_points = geoconv(new_points, new_xyz, num_out_channel, mlp2_bypass[j],
                            mlp2_radius[j], mlp2_radius[j]*2, bn=bn, is_training=is_training,
                            scope='geoconv%d'%(j), bn_decay=bn_decay)
        return new_xyz, new_points




def pointnet_sa_module(xyz, points, npoint, radius,
        radius1, radius2, nsample, mlp, mlp2, group_all,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        b, np, ns, c = new_points.get_shape().as_list()
        new_points = tf.reshape(new_points, [b * np, ns, c])
        grouped_xyz = tf.reshape(grouped_xyz, [b * np, ns, 3])
        for i, num_out_channel in enumerate(mlp):
            new_points = geoconv(new_points, grouped_xyz, num_out_channel, num_out_channel/8,
                            radius1[i], radius1[i]*2, bn=bn, is_training=is_training,
                            scope='conv%d'%(i), bn_decay=bn_decay)

        # Pooling in Local Regions
        new_points = tf.reduce_max(new_points, axis=[1], keep_dims=True, name='maxpool')
        new_points = tf.reshape(new_points, [b, np, -1])

        # [Optional] Further Processing 
        if mlp2 is not None:
            for i, num_out_channel in enumerate(mlp2):
                new_points = geoconv(new_points, grouped_xyz, num_out_channel, num_out_channel/8,
                                radius2[i], radius2[i]*2, bn=bn, is_training=is_training,
                                scope='conv_post_%d'%(i), bn_decay=bn_decay)
        return new_xyz, new_points, idx

def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, gradius, mlp_list, is_training, bn_decay, scope, bn=True, use_xyz=True, use_nchw=False):
    ''' PointNet Set Abstraction (SA) module with Multi-Scale Grouping (MSG)
        Input:
            xyz: (batch_size, ndataset, 3) TF tensor
            points: (batch_size, ndataset, channel) TF tensor
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            b, np, ns, c = grouped_points.get_shape().as_list()
            grouped_points = tf.reshape(grouped_points, [b * np, ns, c])
            grouped_xyz = tf.reshape(grouped_xyz, [b * np, ns, 3])
            for j,num_out_channel in enumerate(mlp_list[i]):
                grouped_points = geoconv(grouped_points, grouped_xyz, num_out_channel, num_out_channel/8,
                                    gradius[j], gradius[j]*2, bn=bn, is_training=is_training,
                                    scope='conv%d_%d'%(i,j), bn_decay=bn_decay)
...
def pointnet_fp_module(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, scope, bn=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            new_points1 = tf_util.perceptron(new_points1, num_out_channel,
...
def pointnet_sa_module_V2(xyz, points, npoint, radius, nsample, mlp, rlist, group_all, is_training, bn_decay,
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i, num_out_channel in enumerate(mlp):
            new_points = geoconv(new_points, new_xyz, num_out_channel, 32,
                    rlist[i], rlist[i]*2, scope="conv%d"%(i))
...
def pointnet_fp_module_V2(xyz1, xyz2, points1, points2, mlp, is_training, bn_decay, scope, bn=True):
    ''' PointNet Feature Propogation (FP) Module
        Input:                                                                                                      
            xyz1: (batch_size, ndataset1, 3) TF tensor                                                              
            xyz2: (batch_size, ndataset2, 3) TF tensor, sparser than xyz1                                           
