...
def conv2d(inputs, num_output_channame, ipt, kernal, chan, relu=True, bn=True):
    conv = O.Conv2D(
        name + '_conv', ipt, kernal, chan,
        nonlinearity=megskull.opr.helper.elemwise_trans.Identity())
    if bn:
        bn = O.BatchNormalization(
            name + '_bn', conv, eps=1e-5, moving_average_fraction=0.99, data_parallel_all_reduce=True)
        affine = O.ElementwiseAffine(name + '_af', bn)
    else:
        affine = conv
...
def conv2d(inputs,
    ...
    """
    conv = O.Conv2D(
        name + '_conv', inputs, kernal_size, num_output_channels,
        nonlinearity=megskull.opr.helper.elemwise_trans.Identity())
    if bn:
        bn = O.BatchNormalization(
            name + '_bn', conv, eps=1e-5, moving_average_fraction=0.99, data_parallel_all_reduce=True)
