...
def extract_pc_in_anchor3d(pc, obj):
    ''' pc: (N,2) '''

    anchor_sizes = {
                'Car': np.array([5, 2, 5]),
                'Pedestrian': np.array([1, 2, 1]),
                'Cyclist': np.array([2, 2, 2])
            }


    center  = np.array(obj.t)
    size    = anchor_sizes[obj.type]

    anchor_mask = (pc <= center + size/2).all(axis=1) * \
                (pc >= center - size/2).all(axis=1)
    anchor_inds = np.where(anchor_mask)
    return pc[anchor_inds,:], anchor_inds
        ...
        for obj_idx in range(len(objects)):
            ...
            for _ in range(augmentX):
                ...
                if perturb_box2d:
                    ...
                    xmin,ymin,xmax,ymax = box2d
                obj = objects[obj_idx]
                pc_in_box_fov, _ = extract_pc_in_anchor3d(pc_rect, obj)
                embed()
