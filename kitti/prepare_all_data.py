...
def process_data(idx_filename, split, output_filename, viz=False,
    ...
    pos_cnt = 0
    all_cnt = 0
    type2onehotclass = {'Car': 1, 'Pedestrian': 2, 'Cyclist': 3}
    for data_idx in data_idx_list:
        ...
        point_label = np.zeros(len(pc_velo))
        ...
        for obj_idx in range(len(objects)):
            ...
            if np.count_nonzero(box_fov_inds) == 0:
                continue
            obj = objects[obj_idx]
            point_label[box_fov_inds] = type2onehotclass[obj.type]
            ...
            if ymax-ymin<25 or (np.sum(label)==0 and obj.type != 'Background'):
                continue
    ...
    with open(output_filename,'wb') as fp:
        ...
        pickle.dump(pc_list, fp)
        pickle.dump(point_label, fp)
        '''
...
def process_data_rgb_detection(det_filename, split, output_filename,
                                       viz=False,
                                       type_whitelist=['Car'],
...
if __name__=='__main__':
    ...
    parser.add_argument('--add_bg', action='store_true', help='randomly generate bg')
    ...
    if args.add_bg:
        type_whitelist.append('Background')
        output_prefix = output_prefix + 'background_'
